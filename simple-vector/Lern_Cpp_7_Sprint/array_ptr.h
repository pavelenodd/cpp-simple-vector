...
public:
  ...
  ArrayPtr(ArrayPtr &&other) { raw_ptr_ = exchange(other.raw_ptr_, nullptr); }
  ...
  /*
  Лучше в случае, если this == &other просто вернуть ссылку на себя,
  в противном случае обменять внутренние указатели (rawptr)
  через std::swap, иначе утечёт старая память.
  */
  ...
  void swap(ArrayPtr &other) noexcept {
    std::swap(other.raw_ptr_, this->raw_ptr_);
  }
